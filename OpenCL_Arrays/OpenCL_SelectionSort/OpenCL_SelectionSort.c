...
int main(){
	...
	fp = fopen("kernel_SelectionSort.cl","r");
	...
	cl_mem a_mem = clCreateBuffer(context,CL_MEM_READ_ONLY,sizeof(int)*N, NULL,&status);
	...
	cl_mem b_mem = clCreateBuffer(context,CL_MEM_WRITE_ONLY,sizeof(int)*N, NULL,&status);
	...
	cl_kernel kernel = clCreateKernel(prgm,"kSelectionSort",&status);
	...
	status = clEnqueueReadBuffer(cq,b_mem,CL_TRUE,0,N*sizeof(int),B,0,NULL,NULL);
